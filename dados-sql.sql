...
--query
SELECT seller.name, SUM(sales.amountAMOUNT ) AS total
FROM tb_seller seller
INNER JOIN tb_sales sales ON seller.id = sales.seller_id
WHERE seller.name = 'Anakin'


SELECT s.id, s.date, SUM(s.amount), tb_seller.name
FROM tb_sales s
INNER JOIN tb_seller ON s.seller_id = tb_seller.id
WHERE s.date BETWEEN '2023-08-01' AND '2024-08-01'
AND tb_seller.name LIKE '%Odi%'
GROUP BY s.id